...
for (GroupName in names(sitesPointsRelative)) {
  ...
  # area_under_curve <- MESS::auc(fit1$x, fit1$y, from = 0, type = 'linear')
  # print(paste("auc of MESS", area_under_curve))
  
  # area_under_curve <- sum(diff(fit1$x) * (head(fit1$y,-1)+tail(fit1$y,-1)))/2
  # print(paste("auc of sum(diff)", area_under_curve))
  ...
  if (enableCI) 
  {
    # tmp <-  data.frame(
    #   x = fit1$x, 
    #   #density = fit1$y / sum(fit1$y),
    #   density= fit1$y/(MESS::auc(fit1$x, fit1$y, from = 0, type = 'linear')),
    #   group = rep(GroupName, times = length(density))#, confidenceDown = NA, confidenceUp = NA
    # )
    ...
    #   CI_ResamplingTime, 
    #   {
    #     sid <- seq_len(length(siteID));
    #     sampledID <- sample(sid, replace=TRUE); 
    #     siteSampledID <- siteID[sampledID];
    #     weightSapmpled<-sitesWeight[sampledID];
    #     #suppressWarnings(density(siteSampledID, adjust = adjust,n=256,weight=weightSapmpled,from=0,to=1)$y)
    #     suppressWarnings(density(siteSampledID, adjust = adjust,from = 0, to = 1,n = 256,weight=weightSapmpled)$y)
    #   }
    # )  #, from=min(fit1$x), to=max(fit1$x)
...
Guitar2Plot <- function(txGTF = NULL,
                            txGFF = NULL,
                            txGenomeVer = NULL,
                            txTxdb = NULL,
                            txGuitarTxdb = NULL,
                            ...
                            miscOutFilePrefix = NA)
{
  ...
  for (i in seq_len(sitesGroupNum)) {
    ...
    for (txType in pltTxType) {
      print(paste("before normalize",format(Sys.time(), "%Y%m%d%H%M%S")))
      ...
      print(paste("after normalize",format(Sys.time(), "%Y%m%d%H%M%S")))
    }
  }
  for (txType in pltTxType)
  {
    ...
    print(paste("before density",format(Sys.time(), "%Y%m%d%H%M%S")))
    ...
    print(paste("after density",format(Sys.time(), "%Y%m%d%H%M%S")))
